...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    def check_duplicates(self, request):
        ...
        return Response({
            ...
            'message': 'Si hay duplicados activos, usa el endpoint fix_duplicates para limpiarlos'
        })

    @action(detail=False, methods=['post'], permission_classes=[IsJefeOperaciones])
    def fix_duplicates(self, request):
        """
        ENDPOINT TEMPORAL para limpiar duplicados de uploaded_file_id.

        Encuentra facturas activas con el mismo uploaded_file_id y:
        - Mantiene la más reciente
        - Marca las demás como eliminadas (soft delete)
        """
        from django.db.models import Count
        ...
        try:
            with transaction.atomic():
                # Encontrar uploaded_file_ids duplicados
                duplicates = Invoice.objects.filter(
                    deleted_at__isnull=True
                ).values('uploaded_file_id').annotate(
                    count=Count('id')
                ...
                for dup in duplicates:
                    ...
                    # Obtener todas las facturas con este uploaded_file_id
                    invoices = Invoice.objects.filter(
                    ...
                    to_delete = list(invoices[1:])  # Todas excepto la primera

                    for invoice in to_delete:
                        invoice.deleted_at = timezone.now()
                        invoice.is_deleted = True
                        invoice.save(update_fields=['deleted_at', 'is_deleted', 'updated_at'])
                        total_cleaned += 1
                    ...
                    cleaned_details.append({
                        ...
                        'kept': {
                            'id': invoices[0].id,
                            'numero_factura': invoices[0].numero_factura,
                ...
                return Response({
                    'success': True,
                    'message': f'Limpieza completada: {total_cleaned} facturas marcadas como eliminadas',
                    'total_cleaned': total_cleaned,
                    'duplicates_fixed': len(cleaned_details),
